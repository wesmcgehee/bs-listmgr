<?php
  /* The Datagrid Class could well be added to the application/library folder,
     but we are going to add it as a helper to the CodeIgniter framework. Why?
     Because loading libraries doesn’t allow us to pass arguments to the class’
     constructor, thus loading it as a helper will solve the problem.
     ref: http://net.tutsplus.com/tutorials/javascript-ajax/build-ajax-data-grids-with-codeigniter-and-jquery-2/
  */
class Datagrid{
	...
	public function generate(){
		...
		foreach($rows as &$row){
			...
			array_unshift($row, "<input class='dg_check_item wijmo-wijdropdown' type='checkbox' name='dg_item[]' value='$id' />");
	...
	public function insertPostSelection(){
	// remove selected items from the db
		if(!empty($_POST['dg_item']))
		    return false;
		else
